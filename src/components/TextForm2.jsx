...
const btnClass =
  "ml-2 whitespace-nowrap inline-flex items-center justify-center px-4 py-2 border border-transparent rounded-sm shadow-sm text-base font-medium text-white bg-sky-600 hover:bg-sky-700 mt-6 float-end";
...
const TextForm2 = (props) => {
  ...
  const listenHandler = () => {
    let listen = document.getElementsByTagName("textarea")[0].value;
  ...
  const downloadHandler = (filename) => {
    ...
    if (textToSave.length > 0) {
      ...
      props.showAlert("No text to download", "Failed");
    }
  };
    ...
    <>
      <div className="max-w-7xl mx-auto mt-4">
        <div className="relative">
          ...
          <button type="button" className={btnClass} onClick={listenHandler}>
            Listen
          </button>
          <button
            type="button"
            className={btnClass}
            onClick={() => downloadHandler("data.txt")}
          >
            Download File
          </button>
        </div>
